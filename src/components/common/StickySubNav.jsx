...
export default function StickySubNav({ onOpenContactModal }) {
  ...
  return (
    <div className="sticky top-14 sm:top-16 z-30 bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 shadow-sm transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-11 sm:h-14">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            {/* Shortlist - hidden on tiny mobile, visible sm+ */}
            <button
              onClick={handleShortlistToggle}
              className={`hidden sm:flex items-center space-x-1.5 px-3 py-1.5 rounded-full border text-xs sm:text-sm font-medium transition-all ${
                isShortlisted
            ...
            >
              <Heart
                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isShortlisted ? "fill-[#E91E63] text-[#E91E63]" : "text-gray-500"}`}
              />
              <span>Shortlist</span>
              <span className="text-xs opacity-75">({shortlistCount})</span>
            </button>

            {/* Share - hidden on tiny mobile, visible sm+ */}
            <button
              onClick={handleShare}
              className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 rounded-full border border-gray-300 dark:border-zinc-700 text-gray-700 dark:text-gray-300 hover:border-[#E91E63] text-xs sm:text-sm font-medium transition-colors"
            >
              {copiedShare ? (
                <>
            ...
            <button
              ...
              className="bg-[#E91E63] hover:bg-[#D81B60] text-white px-3.5 py-1.5 sm:px-5 sm:py-2 rounded-md font-semibold text-xs sm:text-sm shadow-md transition-transform transform active:scale-95 flex items-center space-x-1 sm:space-x-1.5"
            >
